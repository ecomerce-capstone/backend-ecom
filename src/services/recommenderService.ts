...
export async function getPopularProducts(limit = 10) {
  const [rows]: any = await pool.query(
    `SELECT p.id, p.name, p.price, p.vendor_id, p.category_id,
...
export async function getTopProductsByVendor(vendorId: number, limit = 10) {
  const [rows]: any = await pool.query(
    `SELECT p.id, p.name, p.price, p.vendor_id, COALESCE(SUM(oi.quantity),0) AS sold_qty,
...
export async function getUserCategoryDistribution(userId: number, limit = 10) {
  const [rows]: any = await pool.query(
    `SELECT c.id AS category_id, c.name AS category_name, COUNT(*) AS purchases
     FROM orders o
...
export async function getRelatedProducts(productId: number, limit = 10) {
  ...
  if (categoryId) {
    const [sameCat]: any = await pool.query(
      `SELECT p.id, p.name, p.price, p.vendor_id, COALESCE(SUM(oi.quantity),0) AS sold_count, pi.url AS image
       FROM products p
       LEFT JOIN order_items oi ON oi.product_id = p.id
       LEFT JOIN product_images pi ON pi.product_id = p.id AND pi.sort_order = 0
  ...
  const [co]: any = await pool.query(
    `SELECT p.id, p.name, p.price, COALESCE(SUM(oi2.quantity),0) AS co_count, pi.url AS image
...
export async function getRecommendationsForUser(userId: number, limit = 10) {
  ...
  const boughtIds = (boughtRows[0] || []).map((r: any) => r.id);
  ...
  for (const c of categories) {
    ...
    const [rows]: any = await pool.query(
      `SELECT p.id, p.name, p.price, COALESCE(SUM(oi.quantity),0) AS sold_count, pi.url AS image
       FROM products p
       LEFT JOIN order_items oi ON oi.product_id = p.id
       LEFT JOIN product_images pi ON pi.product_id = p.id AND pi.sort_order = 0
