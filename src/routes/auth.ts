...
// //check validation result atau helper validation --old code

// // const handleValidation = (req:express.Request,res:express.Response) => {
...
// //refactor handle validation
// const handleValidation = <T = any>(
//   req: express.Request<any, any, T, any>,
//   //penjelasan :
//   //express.Request punya empat parameter generic (Params, ResBody, ReqBody, ReqQuery)
//   //jadi any,anny,T,any mewakili generic tersebut!
//   // dan default untuk ReqQuery adalah ParsedQs (tipe dari paket qs).
...
//         [fullName.trim(), email.toLowerCase(), hashed]
//       );
...
//   async (req, res) => {
//     if (!handleValidation(req, res)) return;
//     const { name, email, password, phone, storeName, storeDescription } =
//       req.body;
//     try {
//       const [existing]: any = await pool.query(
//         "SELECT id FROM users WHERE email = ?",
...
//           { field: "email", message: "Email already registered as vendor" },
//         ]);
//       const hashed = await hashPassword(password);
//       const [result]: any = await pool.query(
...
//     try {
//       if (role === "vendor") {
//         //maka quesry akan jadi "select id,password from vendors where email= nilai email yg ada di [email-value]"
//         //akan masuk ke row sbgai element array
//         const [rows]: any = await pool.query(
//           "SELECT id,password FROM vendors WHERE email= ?",
//           [email]
//         );
//         if (!rows.length)
...
//           ]);
//         const v = rows[0]; //nilai user,password yg di db
...
//       }
//       //jika bukan vendor maka dia user/customer
//       const [rows]: any = await pool.query(
...
//       const html = `<p>Kami menerima permintaan untuk mereset password Anda. Klik tautan berikut untuk mereset password Anda:</p>`;
